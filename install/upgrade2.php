        ...
        if (!$error) {
            ...
            try {
                ...
                if (!$db->tableExists($table_category)) {
                    ...
                    $db->query($sql);
                    foreach ((array) $config['member_status'] as $status => $department) {
                ...
                if (!$db->tableExists($table_user_meta)) {
                    if ($db->tableExists($table_user_category)) {
                        ...
                        $db->query($sql);
                        $db->query("INSERT INTO `$table` SELECT `status`, 'department', `id` FROM `$table_user`;");
                        $db->query("ALTER TABLE `$db_config[prefix]_edocument` CHANGE `reciever` `department` TEXT NOT NULL");
                        $db->query("ALTER TABLE `$db_config[prefix]_edocument` ADD `receiver` TEXT DEFAULT NULL AFTER `department`");
                        $db->query("ALTER TABLE `$db_config[prefix]_edocument_download` ADD `department_id` VARCHAR(10) DEFAULT NULL");
                        $db->query("UPDATE `$db_config[prefix]_edocument_download` SET `department_id`=(SELECT `status` FROM `$table_user` WHERE `id`=`member_id`)");
                        $db->query("ALTER TABLE `$db_config[prefix]_edocument_download` DROP `id`");
                        $db->query("ALTER TABLE `$db_config[prefix]_edocument` DROP `ip`");
                    }
