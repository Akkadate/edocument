...
function showModal(src, qstr, doClose, className, button) {
  send(src, qstr, function(xhr) {
    ...
    if (detail != '') {
      if (!modal) {
        modal = new GModal({onclose: doClose});
      }
      modal.show(detail, className, button);
      detail.evalScript();
    }
  });
}
...
function initWeb(module) {
  ...
  if ($E('toTop') && !$K.isMobile()) {
    ...
    var header_fixedtop = document.getElementsByClassName('header_fixedtop')[0];
    if (header_fixedtop) {
      toTop -= $G(header_fixedtop).getHeight();
    }
